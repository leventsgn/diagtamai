...
function DiagramCanvasInner() {
  ...
  const runLayout = useCallback(() => {
    ...
    console.log(`🚀 Layout: ${layoutDirection} using ${layoutRanker}`);
  ...
  useEffect(() => {
    if (graphVersion !== lastProcessedVersion.current) {
      ...
      if ((isExternal || isStacked) && nodes.length > 0) {
        console.log("✨ Layout trigger (external or stacked), auto-positioning...");
        runLayout();
      }
    }
  ...
  const categories = [
    { title: "Temel Bileşenler", items: ["process", "decision", "actor", "group", "note", "logic", "rule_engine", "workflow"] },
    ...
    { title: "Altyapı / Cloud", items: ["cloud", "vpc", "subnet", "firewall", "cdn", "dns", "vpn", "region", "zone", "load_balancer_l4", "load_balancer_l7", "global_accelerator", "nat_gateway", "internet_gateway", "peering"] },
    ...
    { title: "İletişim", items: ["pubsub", "webhook", "email", "sms", "notification", "signal", "bus", "mqtt_broker", "socket_io", "kafka_stream", "rabbitmq"] },
    { title: "İş / Harici", items: ["payment_gateway", "crm", "erp", "ecommerce", "marketplace", "api", "stripe", "paypal", "slack_integration", "jira_sync", "salesforce"] },
    { title: "Güvenlik", items: ["shield", "lock", "key", "vpn", "firewall", "iam", "active_directory", "okta", "single_sign_on", "mfa", "encryption", "audit_log"] },
    { title: "Medya / İçerik", items: ["image", "video", "audio", "file", "pdf", "cms", "media_convert", "streaming_server", "bucket_s3"] },
    ...
    <div ref={wrapperRef} style={{ width: "100%", height: "100vh", position: "relative", display: "flex", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 20px 10px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--card-bg)', flexShrink: 0 }}>
          <h3 style={{ margin: 0, fontSize: '15px', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '1px' }}>Kütüphane</h3>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px', background: 'var(--card-bg)' }} className="sidebar-scrollable">
              ...
              <React.Fragment key={cat.title}>
                ...
                {filteredItems.map(t => (
                  <button
                    ...
                    title="Sürükleyip bırakabilirsin"
                  >
          ...
          {searchTerm && searchTerm.length >= 2 && (
            <>
              <div className="sidebar-cat-title">Tüm Kütüphane Sonuçları</div>
              {Object.keys(Icons)
                .filter(iconName =>
